...
if (isset($_GET['disease']) && $_GET['disease'] != '') {
    ...
    $countQuery = "SELECT COUNT(*) AS patientCount, YEAR(creationDate) AS creationYear, age
            FROM patients 
            WHERE disease = $selectedDisease AND YEAR(creationDate) = $selectedYear
            GROUP BY age";
...
<div class="card shadow">
    <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
        <h6 class="m-0 font-weight-bold text-primary">Yearly age distribution per disease line chart</h6>
...
<script>
    ...
    downloadButton.addEventListener("click", () => {
        ...
        link.href = imageUrl;
        link.download = `${diseaseName} age line chart.png`;
        link.click();
    });
</script>